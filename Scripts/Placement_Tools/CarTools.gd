...
func _ready():
	...
	randomize_colors_exec(true)

func randomize_colors_exec(dummy: bool) -> void:
	car_mesh.set_surface_override_material(0, car_colors[randi_range(0, car_colors.size()-1)])
	if lights_on:
		car_mesh.set_surface_override_material(3, mat_front_light_on)
		car_mesh.set_surface_override_material(4, mat_back_light_on)
		for light in car_lights:
			light.visible = true
	else:
		car_mesh.set_surface_override_material(3, mat_front_light_off)
		car_mesh.set_surface_override_material(4, mat_back_light_off)
		for light in car_lights:
			light.visible = false
